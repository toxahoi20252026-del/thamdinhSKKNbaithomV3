...
function MainApp() {
  ...
  // Danh sách tài khoản cấu hình cứng
  const USERS_DB: { username: string; password: string; fullName: string; role: 'admin' | 'judge' }[] = [
    { username: 'tonghoanglinh', password: '23011980aA@', fullName: 'Tống Hoàng Linh', role: 'admin' },
    { username: 'nguyenvantrung', password: '02101978', fullName: 'Nguyễn Văn Trung', role: 'judge' },
    { username: 'lethibichphuong', password: '04111979', fullName: 'Lê Thị Bích Phượng', role: 'judge' },
    { username: 'nguyenthanhlong', password: '08091979', fullName: 'Nguyễn Thanh Long', role: 'judge' },
    { username: 'daophian', password: '17091986', fullName: 'Đào Thị Phi An', role: 'judge' },
    { username: 'nguyenthilua', password: '05121987', fullName: 'Nguyễn Thị Lụa', role: 'judge' },
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Giả lập delay đăng nhập cho UX mượt mà
  ...
  const chartData = useMemo(() => {
    const categories = [
      { name: 'Giỏi (8-10)', value: initiatives.filter(i => i.score >= 8).length, color: '#10B981' },
      { name: 'Khá (6.5-7.9)', value: initiatives.filter(i => i.score >= 6.5 && i.score < 8).length, color: '#6366F1' },
      { name: 'Đạt (6-6.4)', value: initiatives.filter(i => i.score >= 6 && i.score < 6.5).length, color: '#F59E0B' },
      { name: 'Chưa đạt (<6)', value: initiatives.filter(i => i.score < 6).length, color: '#F43F5E' },
    ].filter(c => c.value > 0);
          ...
          <div className="max-w-7xl mx-auto w-full h-full flex items-center justify-between px-4 md:px-10">
            ...
            <div className="flex items-center space-x-3 md:space-x-6">
              <a 
                ...
                className="hidden lg:flex items-center space-x-1.5 px-3 py-1.5 bg-red-50 hover:bg-red-100 border border-red-200 rounded-full transition-colors group"
              >
                <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
                <span className="text-[10px] font-bold text-red-600 uppercase tracking-tight">Lấy API key để sử dụng app</span>
              </a>
              <button
                onClick={() => confirm('Bạn có chắc chắn muốn đăng xuất?') && handleLogout()}
                ...
                <div className="glass-card p-8 space-y-6">
                  ...
                  <form onSubmit={handleSaveSettings} className="space-y-5">
                    ...
                    <div>
                      ...
                      >
                        <option value="gemini-3-flash-preview">Gemini 3 Flash (Mặc định - Nhanh)</option>
